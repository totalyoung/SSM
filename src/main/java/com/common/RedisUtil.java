...
public class RedisUtil {
	...
	private static RedisTemplate<String,Object> redisTemplate = (RedisTemplate<String, Object>) SpringContextUtil.getBeanById("redisTemplate");
	

	private static StringRedisTemplate stringRedisTemplate = (StringRedisTemplate) SpringContextUtil.getBeanById("stringRedisTemplate");
    ...
    public static <T> T getJson(String key, Class<T> clazz) {  
        return JsonUtil.nonDefaultMapper().toBean(stringRedisTemplate.boundValueOps(key).get(), clazz);  
    }  
    ...
    public static void setJson(String key,Object value,long time){  
        stringRedisTemplate.opsForValue().set(key, JsonUtil.nonDefaultMapper().toJson(value));  
        if(time > 0){  
    ...
    public static void setJsonField(String key, String field, String value){  
    	ObjectNode jsonToNode = JsonUtil.nonDefaultMapper().jsonToNode(stringRedisTemplate.boundValueOps(key).get());
    	jsonToNode.put(field, value);
        stringRedisTemplate.opsForValue().set(key, jsonToNode.toString());  
    }  
    ...
    public static <T> void setMap(String key, T obj, long time){  
        Map<String, Object> map = (Map<String, Object>)JsonUtil.nonDefaultMapper().beanToMap(obj);  
        redisTemplate.opsForHash().putAll(key, map);  
    }  
    ...
    public static <T> T getMap(String key, Class<T> clazz){  
        ...
        return JsonUtil.nonDefaultMapper().mapToBean(map, clazz);  
    }  
